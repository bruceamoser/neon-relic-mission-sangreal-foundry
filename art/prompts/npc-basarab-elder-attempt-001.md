# npc-basarab-elder — npc-basarab-elder
...
Medium: Photographic environmental portrait; honor the asset-specific setting and illumination, 4:5, keep the face and eyes usable for Foundry square crops without excluding mandatory props.

Aspect ratio: 4:5. Minimum short edge 1024 px, prefer 1536–2048 px.
...
Canonical character basarab-elder: vampire elder presenting as late 60s, aristocratic, aquiline nose, swept-back grey-white hair, deep facial lines, long black wool greatcoat with a high collar, signet ring bearing a small dragon-wolf crest, regarding the viewer with mild, patient curiosity, lit by candlelight only

Reference inputs (identity/prop references, never panels or inserted pictures):

- ref-basarab-crest: /mnt/disk4/repos/neon-relic-mission-sangreal-foundry/art/references/ref-basarab-crest.webp

Asset prompt (verbatim): Character portrait of the BASARAB ELDER (see reference) standing in a candle-lit stone cellar, hands clasped behind his back; candlelight only, deep chiaroscuro; expression: mild, patient curiosity, as if examining furniture.

Scene requirements: 

Mandatory visible Eggs:

- npc-basarab-elder-E1: a dragon-wolf signet ring

- npc-basarab-elder-E2: a Wallachian boyar's fur collar under the coat, three centuries out of date.
...
Reference usage: supplied image is only the local prop/motif reference. Do not copy its background, ruler or evidence marker into the portrait. Motif glossary is context, not a list of objects to add; include only this portrait's requested props.
Use a three-quarter side angle so clasped hands behind his back peek past the hip outline and the small signet ring is visible naturally. Preserve the requested behind-back posture and visible fur collar.
