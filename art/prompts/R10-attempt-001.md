...
Asset prompt (verbatim): Top-down photograph of a diocesan road map of Northern Italy, partially unrolled beside its worn leather casing, an embossed stamp pressed into the hide showing an open eye inside a serpent biting its own tail with tiny initials T.O.W., faint pencil circles on the map around Geneva, Zurich and Turin, warm desk lamp, shallow depth of field, film grain.

Scene requirements: Claudia's diocesan road map of Northern Italy partially unrolled, leather casing below; the embossed stamp pressed into the hide: open eye within an ouroboros, tiny initials `T.O.W.`

Mandatory visible Eggs:

- R10-E1: the map has three locations circled faintly in pencil that match the operational clock: Geneva, Zurich, Turin

- R10-E2: the casing stitching hides a second, fainter impression — the stamp has been used before.
