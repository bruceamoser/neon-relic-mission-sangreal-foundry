#!/usr/bin/env node
/**
 * Mission: Sangreal — pack validation tooling.
 *
 ...
 * alongside `npm run build` in every content issue.
 *
 * Usage:
...
/**
 * Verify that a module-relative asset path (image or audio) exists in `src/assets`.
 * Paths pointing outside the module (system/core assets) are ignored.
 * @param {unknown} p
 * @param {string} label
 * @param {string[]} errors
...
async function main() {
  ...
  if (errors.length > 0) {
    console.error(`mission-sangreal | validation FAILED (${errors.length} issue(s)):`);
    for (const e of errors) console.error(`  ✖ ${e}`);
    process.exit(1);
  }

  console.log(`mission-sangreal | validation passed — ${files.length} pack files, ${docCount} documents`);
}

main().catch((err) => {
  console.error('mission-sangreal | validation crashed');
  console.error(err);
  process.exit(1);
});
